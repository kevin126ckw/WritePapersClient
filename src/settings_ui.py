...
class SettingsDialog(tk.Toplevel):
    ...
    def __init__(self, parent, settings_items):
        """
        初始化设置对话框
        :param parent: 父窗口
    ...
    def create_settings_widgets(self, parent_frame):
        ...
        for item in self.settings_items:
            ...
            # 创建标签
            label = ttk.Label(frame, text=item["label"] + ":", width=15, anchor=tk.W)
    ...
    def create_buttons(self, parent_frame):
        ...
        # 取消按钮
        btn_cancel = ttk.Button(btn_frame, text="取消", width=10, command=self.destroy)
...
if __name__ == "__main__":
    ...
    def open_settings():
        """打开设置对话框的示例函数"""
        dialog = SettingsDialog(root, settings_config)
        root.wait_window(dialog)  # 等待对话框关闭
        print("最终设置:", dialog.get_settings())
