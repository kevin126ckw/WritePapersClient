...
def load_messages(contact, display_message):
    ...
    if uid is None:
        ...
        uid = 0
    if contact["id"] == int(uid):
        gui.show_toast("解锁成绩：给自己发消息？",position="top-right")
        messages = db.select_sql("chat_history", "*", f"to_user={uid} and from_user={uid}")
    else:
...
def welcome_back():
    while net.welcome_back_queue.empty():
        pass
    welcome_back_msg = net.welcome_back_queue.get_nowait()
    gui.show_toast(welcome_back_msg['payload']['message'], position="top-right", toast_type="success")
...
def send_picture(gui_class, contact):
    ...
    if image_path:
        ...
        def _send_picture():
            # 添加到消息记录
            logger.debug("正在发送图片数据")
            net.send_packet("send_message", {"to_user": str(contact["id"]), "type": "image",
                                             "message": base64.b64encode(image_data).decode("utf-8")})# Base64编码图片数据
            logger.debug("图片数据发送完成")
...
def main():
    ...
    gui.root.after(100, welcome_back)
    root.mainloop()
